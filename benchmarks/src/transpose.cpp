	...
	if (jsonContains(options, "librapid")) {
		...
		for (const auto &size : sizes) {
			...
			if (dtype == "f32") {
				...
				bench = lrc::timeFunction([&]() { res = lhs.transposed(); }, samples, iters, time);
			} else if (dtype == "f64") {
	...
	if (jsonContains(options, "eigen")) {
		...
		for (const auto &size : sizes) {
			...
			if (dtype == "f32") {
				...
				bench = lrc::timeFunction([&]() { res = lhs.transpose(); }, samples, iters, time);
			} else if (dtype == "f64") {
	...
	if (jsonContains(options, "xtensor")) {
		...
		for (const auto &size : sizes) {
			...
			if (dtype == "f32") {
				...
				bench = lrc::timeFunction([&]() { res = xt::transpose(lhs); }, samples, iters, time);
			} else if (dtype == "f64") {
